...
export default function Radio({ theme, options, length }: { theme: string, options: string[], length: number }) {
  ...
  function nextQuestion(formData: FormData) {
    ...
    return (
        ...
        {options.map((option, index) => (
          <label key={index}>
            <input className="ml-[15px]" type="radio" name={theme} value={option} /> {option}
          </label>
        ))}
